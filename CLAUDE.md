...
그룹 밖에 페이지를 만들지 않는다. 루트 `/` 경로는 홈 페이지이며, `(guest)/(with-menu)` 그룹 안의 `page.tsx`로 존재한다.

## 버전 관리

### 2. PR merge 시 버전 업데이트
...
### 3. 모바일 전용 · iPhone 14 Pro Max (430 × 932) 기준
...
### 4. 새로운 패키지/라이브러리 도입 금지 (사전 승인 필수)
...
- 새 HTTP/상태관리 라이브러리 도입은 의존성 관리 규칙(#4, 사전 승인)을 따른다.
