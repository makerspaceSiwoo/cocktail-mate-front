...
**유일한 예외:** `src/app/sitemap/page.tsx` (`/sitemap` — 개발용 라우트 인덱스). 이 파일은 개발용 전체 경로 목록 페이지로, 그룹에 속하지 않는다. 이 외에 그룹 밖에 페이지를 만들지 않는다. 루트 `/` 경로는 홈 페이지이며, `(guest)/(with-menu)` 그룹 안의 `page.tsx`로 존재한다.

### 2. 신규 페이지 추가 시 sitemap 페이지 업데이트

페이지를 추가하면 `src/app/sitemap/page.tsx`의 `ROUTE_TREE` 배열에 해당 경로를 반드시 추가한다. tree 형태의 UI로 그룹별 소속과 페이지 간 연결 관계를 한눈에 파악할 수 있도록 한다.
