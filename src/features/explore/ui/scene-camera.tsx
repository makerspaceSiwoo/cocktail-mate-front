...
/**
 * 카메라 배치 + TrackballControls.
 *
 * - 구 전체가 한 눈에 보이도록 카메라 거리를 계산해 배치한다(좌우 여백 확보).
 *   뷰포트 크기가 바뀌면 다시 맞춘다.
 * - TrackballControls: 회전 + 줌만 허용(noPan=true). 줌 한계는 fit 거리 기준.
 */
export function SceneCamera() {
  ...
  useLayoutEffect(() => {
    // 위치만 이동(회전은 TrackballControls 가 관리). near/far 는 Canvas 기본값(0.1/100)
  ...
  return (
    <TrackballControls
      ...
      // 1. 이동(Pan) 비활성화
      noPan={true}
      // 2. 부드러운 관성(Damping) 설정
      staticMoving={false}
      dynamicDampingFactor={0.15}
      // 3. 줌 제한 (기본 fit 거리를 최대 거리로 두어 줌 아웃 방지, 줌 인만 허용)
      minDistance={dist * 0.5}
      maxDistance={dist}
      // 4. 회전 및 줌 감도 조절 (Trackball에 맞게 보정된 감도)
      rotateSpeed={1.0}
      zoomSpeed={1.5}
    />
  );
